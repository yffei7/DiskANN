...
static void read_trace_file(const std::string &path, std::vector<uint32_t> &delete_ids,
  ...
  if (n < 0) {
    throw std::runtime_error("invalid trace size: " + path);
  }
...
static double compute_recall_from_gt(const std::string &truthset_file,
  ...
  if (gt_num != query_num) {
    ...
    delete[] gt_dists;
    delete[] gt_tags;
    throw std::runtime_error("truthset/query size mismatch");
  }
...
static void append_row(std::ofstream &out, const std::string &system,
                       const std::string &workload, uint32_t checkpoint_id,
                       uint32_t round_index, uint32_t base_points,
                       uint32_t active_points, uint32_t cumulative_inserts,
                       uint32_t cumulative_deletes, uint32_t recall_at,
...
static int run_fair(const std::string &workload, const std::string &base_bin,
  ...
  size_t dim = 0;
  {
    ...
    std::ifstream in(base_bin, std::ios::binary);
    if (!in) {
      throw std::runtime_error("failed to open base bin: " + base_bin);
    }
    in.read(reinterpret_cast<char *>(&npts_i32), sizeof(int32_t));
    in.read(reinterpret_cast<char *>(&dim_i32), sizeof(int32_t));
    if (dim_i32 <= 0) {
      throw std::runtime_error("invalid base dim in: " + base_bin);
    }
  ...
  diskann::MergeInsert<T, uint32_t> index(params, dim, index_prefix + "_mem",
                                          index_prefix, index_prefix + "_merge",
  ...
  if (workload_has_query(workload)) {
    if (workload == "query_only") {
      for (uint32_t checkpoint = 0; checkpoint < std::max<uint32_t>(1, query_only_checkpoints); ++checkpoint) {
        ...
        append_row(out, "diskann", workload, checkpoint, 0, base_points, active_points,
    ...
    uint64_t disk_size = disk_index_size_bytes(index.ret_merge_prefix());
    uint64_t aux_size = aux_index_artifact_bytes(index.ret_merge_prefix());
    append_row(out, "diskann", workload, 0, 0, base_points, active_points,
               cumulative_inserts, cumulative_deletes, recall_at, search_L,
               beamwidth, search, BatchMetrics(), disk_size, aux_size, io_start, io_prev, io_cur,
               current_mem_kb(), "native_online", false, 0.0, 0.0, 0,
               "maintenance_mode=native_online");
...
#pragma omp parallel for num_threads(std::max<uint32_t>(1, insert_threads)) schedule(runtime)
    ...
    uint64_t disk_size = disk_index_size_bytes(index.ret_merge_prefix());
    uint64_t aux_size = aux_index_artifact_bytes(index.ret_merge_prefix());
    append_row(out, "diskann", workload, round + 1, round + 1, base_points,
               active_points, cumulative_inserts, cumulative_deletes, recall_at,
               search_L, beamwidth, search, batch, disk_size, aux_size, io_start, io_prev, io_cur,
               current_mem_kb(), "native_online", maintenance_triggered, foreground_s, maintenance_s, pending_updates,
               workload == "update_only"
                   ? "maintenance_mode=native_online;recall_disabled;merge_policy=mem_ratio_3pct"
                   : "maintenance_mode=native_online;merge_policy=mem_ratio_3pct");
    io_prev = io_cur;
  }
...
int main(int argc, char **argv) {
  if (argc != 34) {
    std::cout << "Usage: " << argv[0]
              ...
              << " <build_B> <build_M> <build_T> <alpha> <nodes_to_cache> <merge_trigger_ratio> <query_only_checkpoints> <reuse_existing_index>"
              << std::endl;
    return -1;
  }
  ...
  float alpha = static_cast<float>(std::atof(argv[29]));
  unsigned nodes_to_cache = static_cast<unsigned>(std::stoul(argv[30]));
  double merge_trigger_ratio = std::atof(argv[31]);
  uint32_t query_only_checkpoints = static_cast<uint32_t>(std::stoul(argv[32]));
  bool reuse_existing_index = std::stoi(argv[33]) != 0;
  ...
  try {
    if (type == "float") {
      return run_fair<float>(workload, base_bin, full_bin, query_bin, trace_prefix, gt_prefix,
                             ...
                             build_L, build_R, alpha, build_L, build_R, alpha, nodes_to_cache,
                             merge_trigger_ratio,
