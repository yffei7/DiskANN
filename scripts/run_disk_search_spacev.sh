...
# Microsoft SPACEV dataset (100M points, dimension 100, int8).
#
# Usage:
#   bash scripts/run_disk_search_spacev.sh <data_file.bin> <query_file.bin> \
#       <index_prefix> <result_prefix> [groundtruth.bin]
#
# Parameters tuned for SPACEV 100M (single-threaded search):
#   R=64, L=100, B=20 (20 GB in-memory budget), M=64 (64 GB build RAM), T=64
#   num_nodes_to_cache=500000, num_threads=1, beamwidth=4
#   L_search values: 100 150 200
#
# NOTE: The SPACEV dataset uses int8 vectors. Make sure your data file is in
# DiskANN's .bin format with int8 values. Use tests/utils/float_bin_to_int8
# if you need to convert from float.
#
# NOTE: B and M depend on the RAM available on your machine.
#   - B: the portion of the index kept in RAM during search (GB). The raw
#        SPACEV data is ~10 GB (100M * 100 * 1 byte), so B=20 is generous.
...
BUILD_RAM_GB=64    # M: RAM budget for building (GB)
SEARCH_RAM_GB=20   # B: in-memory budget for search index (GB)
...
echo "=== Building disk index for SPACEV 100M ==="
"$BUILD_BIN" int8 "$DATA_FILE" "$INDEX_PREFIX" \
...
#            <L1> [L2] ...
#   num_nodes_to_cache=500000 : cache top BFS nodes in RAM
...
echo "=== Searching disk index for SPACEV 100M (single-threaded) ==="
"$SEARCH_BIN" int8 "$INDEX_PREFIX" 0 0 500000 1 4 \
